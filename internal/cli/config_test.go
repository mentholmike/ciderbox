...
`), 0o600); err != nil {
	...
	if cfg.CFContainers.APIURL != "https://cf-containers.example.test" || cfg.CFContainers.Token != "cf-containers-token" || cfg.CFContainers.Workdir != "/workspace/cf-test" {
		t.Fatalf("cf containers config not loaded: %#v", cfg.CFContainers)
	}
...
func TestEnvOverridesConfig(t *testing.T) {
	...
	if cfg.CFContainers.APIURL != "https://cf-containers-env.example" || cfg.CFContainers.Token != "cf-containers-env-token" || cfg.CFContainers.Workdir != "/workspace/cf-containers-env" {
		t.Fatalf("unexpected cf containers env: %#v", cfg.CFContainers)
	}
